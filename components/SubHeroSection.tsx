...
function SubHeroSection() {
    return (
        <div className="w-full h-full flex flex-wrap items-center justify-center gap-7">
            {cards.map((card) => (
                <div key={card.id} className="pl-15 pr-15 flex flex-col justify-center items-center gap-3 border-1 border-[#262626] p-5 rounded-lg">
                    ...
                    <h2 className="text-white text-[1rem]">{card.title}</h2>
                </div>
            ))}
        </div>
    );
